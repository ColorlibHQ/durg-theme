...
function durg_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'durg' ),
        'next_string'     => esc_html__( '&raquo;', 'durg' ),
        'before_output'   => '<div class="durg-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'durg_pagination_defaults', $defaults )
    );
