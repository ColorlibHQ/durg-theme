...
// enqueue css
function durg_common_custom_css(){
    
    wp_enqueue_style( 'durg-common', get_template_directory_uri().'/assets/css/common.css' );

		$themeColor     		  = durg_opt( 'durg_theme_color' );
		$themeSecColor     		  = '';
		$buttonHoverBgColor       = durg_opt( 'durg_quote_btn_bg_color' ) != '#ff5e13' ? durg_opt('durg_quote_btn_bg_color') : $themeColor;
		$buttonBorderColor     	  = durg_opt( 'durg_quote_btn_hvr_text_color' ) != '#ff5e13' ? durg_opt('durg_quote_btn_hvr_text_color') : $themeColor;;
		$hoverColor     	  	  = durg_opt( 'durg_quote_btn_hvr_text_color');

		$headerTop_bg     		  = durg_opt( 'durg_top_header_bg_color' );
		$headerTop_col     		  = durg_opt( 'durg_top_header_color' );

		$headerTopBg      		  = durg_opt( 'durg_top_header_bg_color');
		$headerBg          		  = durg_opt( 'durg_header_bg_color') != '#ffffff' ? durg_opt( 'durg_header_bg_color') : '';
		$menuColor          	  = durg_opt( 'durg_header_menu_color' ) != '#191d34' ? durg_opt('durg_header_menu_color') : '';
		$menuHoverColor           = durg_opt( 'durg_header_menu_hover_color' ) != '#ff5e13' ? durg_opt('durg_header_menu_hover_color') : $themeColor;
		$menuFixedHoverColor      = durg_opt( 'durg_header_menu_fixed_hover_color' );
		$dropMenuBgColor          = durg_opt( 'durg_header_drop_menu_bg_color' ) != '#ffffff' ? durg_opt('durg_header_drop_menu_bg_color') : $themeSecColor;
		$dropMenuColor            = durg_opt( 'durg_header_drop_menu_color' );
		$dropMenuHovColor         = durg_opt( 'durg_header_drop_menu_hover_color' );

		$footerwbgColor     	  = durg_opt('durg_footer_bg_color') != '#ffffff' ? durg_opt('durg_footer_bg_color') : '';
		$footerwHeadColor   	  = durg_opt('durg_footer_widget_heading_color');
		$footerwTextColor   	  = durg_opt('durg_footer_widget_text_color');
		$footerwanchorcolor 	  = durg_opt('durg_footer_widget_anchor_color') != '#5c5c5c' ? durg_opt('durg_footer_widget_anchor_color') : '';
		$footerwanchorhovcolor    = durg_opt('durg_footer_widget_anchor_hover_color') != '#ff5e13' ? durg_opt('durg_footer_widget_anchor_hover_color') : $themeSecColor;
		
		$fofbg					  = durg_opt('durg_fof_bg_color');
		$foftonecolor			  = durg_opt('durg_fof_textone_color');
		$fofttwocolor			  = durg_opt('durg_fof_texttwo_color');

		$bannerBtnHeaderSpanColor = $themeColor != '#ff5e13' ? $themeColor : '';
		$footerAncDefColor 		  = $footerwanchorcolor != '#ff5e13' ? $footerwanchorcolor : $themeColor;
		$footerAncDefHovColor 	  = $footerwanchorhovcolor != '#ff5e13' ? $footerwanchorhovcolor : $themeSecColor;

		$customcss ="			
			.header-area .main-header-area .quote-area .get-quote .boxed-btn
			{
				background-color: {$buttonHoverBgColor};
			}
			.header-area .main-header-area .quote-area .get-quote .boxed-btn:hover
			{
				color: {$buttonBorderColor} !important;
			...
			.btn_4:hover, .about-area .section-title .boxed-btn:hover, .form-contact .form-group .btn_1:hover, .f0f-content .btn_1:hover, .search-page .backtohome .btn_1:hover, .contact-section .button-contactForm:hover{
				color: {$themeColor}!important;
			}
			...
			.about-area .section-title .boxed-btn:hover, .form-contact .form-group .btn_1:hover, .f0f-content .btn_1:hover, .search-page .backtohome .btn_1:hover, .contact-section .button-contactForm:hover {
				background: transparent;
			}
			...
			.footer-area .single-footer-widget h4, .footer-area .single-footer-widget .office-location ul li strong
			{
				color: {$footerwHeadColor}
			}
			.footer-area .single-footer-widget p, .footer-area .widget_durg_newsletter .input-group input, .footer-area .copyright_part_text p, .footer_Part .copyright_part p, .footer-area .footer_2 .social_icon a, .footer_Part .footer_text h2, .footer-area .single-footer-widget .office-location ul li p
			{
			...
			.single_sidebar_widget.widget_search .boxed-btn:hover {
				color: #ffffff !important;
			}

        ";
       
    wp_add_inline_style( 'durg-common', $customcss );
    
}
add_action( 'wp_enqueue_scripts', 'durg_common_custom_css', 50 );