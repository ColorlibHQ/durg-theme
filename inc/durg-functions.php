...
function durg_meta( $id = '' ){
    
    $value = get_post_meta( get_the_ID(), '_durg_'.$id, true );
    
    return $value;
}
// Blog Date Permalink
function durg_blog_date_permalink(){
...
// Blog Excerpt Length
if ( ! function_exists( 'durg_excerpt_length' ) ) {
	function durg_excerpt_length( $limit = 30 ) {
...
if ( ! function_exists( 'durg_posted_comments' ) ) :
    function durg_posted_comments(){
        
        $comments_num = get_comments_number();
        if( comments_open() ){
            if( $comments_num == 0 ){
                $comments = esc_html__('No Comments','durg');
            } elseif ( $comments_num > 1 ){
                $comments= $comments_num . esc_html__(' Comments','durg');
            } else {
                $comments = esc_html__( '1 Comment','durg' );
            }
            $comments = '<a href="' . esc_url( get_comments_link() ) . '">'. $comments .'</a>';
        } else {
            $comments = esc_html__( 'Comments are closed', 'durg' );
        }
...
function durg_iframe_match(){   
    $audio_content = durg_embedded_media( array('audio', 'iframe') );
    $iframe_match = preg_match("/\iframe\b/i",$audio_content, $match);
    return $iframe_match;
}

//Post embedded media
function durg_embedded_media( $type = array() ){
...
// WP post link pages
function durg_link_pages(){
    
    wp_link_pages( array(
    'before'      => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'durg' ) . '</span>',
    'after'       => '</div>',
    'link_before' => '<span>',
    'link_after'  => '</span>',
    'pagelink'    => '<span class="screen-reader-text">' . esc_html__( 'Page', 'durg' ) . ' </span>%',
    'separator'   => '<span class="screen-reader-text">, </span>',
    ) );
}

// Blog social sharing buttons
function durg_social_sharing_buttons( $ulClass = '', $tagLine = '' ) {
...
if( ! function_exists('durg_blog_single_post_navigation') ) {
	function durg_blog_single_post_navigation() {
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-12 nav-left flex-row d-flex justify-content-start align-items-center">
						...
						if( get_next_post_link() ){
							...
							if( has_post_thumbnail() ){
								...
								<div class="thumb">
									<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
										<?php echo get_the_post_thumbnail( absint( $nextPost->ID ), 'durg_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
									</a>
								</div>
								<?php
							} ?>
							...
							<div class="detials">
								<p><?php echo esc_html__( 'Prev Post', 'durg' ); ?></p>
								<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
					...
					<div class="col-lg-6 col-md-6 col-12 nav-right flex-row d-flex justify-content-end align-items-center">
						...
						if( get_previous_post_link() ){
							...
							?>
							<div class="detials">
								<p><?php echo esc_html__( 'Next Post', 'durg' ); ?></p>
							...
							<div class="thumb">
								<a href="<?php the_permalink(  absint( $prevPost->ID )  ) ?>">
									<?php echo get_the_post_thumbnail( absint( $prevPost->ID ), 'durg_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
								</a>
							</div>
							<?php
						} ?>
...
// Durg Comment Template Callback
function durg_comment_callback( $comment, $args, $depth ) {
				...
				<div class="desc">
					...
					<div class="d-flex justify-content-between">
						<div class="d-flex align-items-center">
							<h5 class="comment_author"><?php printf( __( '<span class="comment-author-name">%s</span> ', 'durg' ), get_comment_author_link() ); ?></h5>
							<p class="date"><?php printf( __('%1$s at %2$s', 'durg'), get_comment_date(),  get_comment_time() ); ?><?php edit_comment_link( esc_html__( '(Edit)', 'durg' ), '  ', '' ); ?> </p>
							<?php if ( $comment->comment_approved == '0' ) : ?>
								<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'durg' ); ?></em>
								<br>
							<?php endif; ?>
						</div>
...
add_filter('comment_reply_link', 'durg_replace_reply_link_class');
function durg_replace_reply_link_class( $class ) {
...
// theme logo
function durg_theme_logo( $class = '' ) {

    $siteUrl = home_url('/');
    // site logo
		
	$custom_logo_id = get_theme_mod( 'custom_logo' );
	$imageUrl = wp_get_attachment_image_src( $custom_logo_id , 'durg_site_logo_106x34' );
	
	if( !empty( $imageUrl[0] ) ){
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( $imageUrl[0] ).'" alt="'.esc_attr( durg_image_alt( $imageUrl[0] ) ).'"></a>';
	}else{
...
function durg_pull_right( $id = '', $condation ){
...
function durg_get_textareahtml_output( $content ) {
...
function durg_get_slider_shortcode_options( $field ) {
...
// html Style tag
function durg_inline_bg_img( $bgUrl ){
    $bg = '';
...
function durg_sidebar_opt(){

    $sidebarOpt = durg_opt( 'durg-blog-sidebar-settings' );
    $sidebar = '1';
...
function durg_blog_pagination(){
	echo '<nav class="blog-pagination justify-content-center d-flex">';
        the_posts_pagination(
            array(
                'mid_size'  => 2,
                'prev_text' => __( '<span class="ti-angle-left"></span>', 'durg' ),
                'next_text' => __( '<span class="ti-angle-right"></span>', 'durg' ),
                'screen_reader_text' => ' '
            )
        );
	echo '</nav>';
...
// Themify Icon
function durg_themify_icon(){
    return[
        'flaticon-layers'     => __('Layer Icon', 'durg'),
        'flaticon-design'     => __('Design Icon', 'durg'),
        'flaticon-speaker'    => __('Speaker Icon', 'durg'),
    ];
}

// CTA section
function durg_get_cta_area() {
	$cta_section_title      = !empty( durg_opt( 'durg_cta_section_title') ) ? durg_opt( 'durg_cta_section_title') : '';
	$cta_section_text       = !empty( durg_opt( 'durg_cta_section_text') ) ? durg_opt( 'durg_cta_section_text') : '';
	$cta_section_btn_label  = !empty( durg_opt( 'durg_cta_section_btn_label') ) ? durg_opt( 'durg_cta_section_btn_label') : '';
	$cta_section_btn_url    = !empty( durg_opt( 'durg_cta_section_btn_url') ) ? durg_opt( 'durg_cta_section_btn_url') : '';
	?>
    <!-- cta-area-start -->
...
function durg_get_contact_details_area() {
	?>
    <!-- location-area-start -->
    <div class="addres-area black-bg section-padding">
        <div class="container">
            <div class="row">
                <?php
				// Contact items
				$contact_items = durg_opt('durg_contact_details_section_fields');
                if ( $contact_items == true ) {
                    ...
                    if( is_array( $contact_items ) && count( $contact_items ) > 0 ){
                        foreach ($contact_items as $value) {
                            switch ($counter) {
                                case $counter == 2:
                                    $icon_img = DURG_DIR_ICON_IMG_URI . 'clock.png';
                                    break;
                                case $counter == 3:
                                    $icon_img = DURG_DIR_ICON_IMG_URI . 'envelope.png';
                                    break;
                                default:
                                    $icon_img = DURG_DIR_ICON_IMG_URI . 'globe.png';
                                    break;
                            }
                            $counter++;
